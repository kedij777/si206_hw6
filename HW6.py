...
def load_json(filename):
    ...
    ----------
    filename: string
        the name of the cache file to read in - path
...
def cache_all_pages(people_url, filename):
    ...
    load = load_json(filename)
    dict = {}
    for i in range(1,10):
        url = people_url + "?page=" + str(i)
        if "page " + str(i) not in load:
            tempList = []
            tempList.append(get_swapi_info(url))
            dict["page " + str(i)] = tempList
...
def get_starships(filename):
    ...
    Returns
    -------
    dict: dictionary with the character's name as a key and a list of the name their starships as the value
    '''
    dict = load_json(filename)
    out = {}
    for page in dict:
        for person in dict[page][0]["results"]:
            starships = person['starships']
            if len(starships) == 0:
                continue
            name_of_starships = []
            for url in starships:
                starship_dict = get_swapi_info(url,"temp_cache.json")
                name_of_starships.append(starship_dict["name"])
            out[person["name"]] = name_of_starships
    return out
...
def calculate_bmi(filename):
    ...
    for page in dict:
        for person in dict[page][0]["results"]:
            ...
            # 1,764
            if "," in mass or "," in height:
                cleanedMass = mass.replace(',', '')
                cleanedHeight = height.replace(',', '')
            else:
                cleanedMass = mass
                cleanedHeight = height
            
            # int
            if cleanedMass.isdigit() and cleanedHeight.isdigit():
                BMI = int(cleanedMass) / (int(cleanedHeight) ** 2) * 10000
                out[person["name"]] = round(BMI, 2)
            elif "." in cleanedMass or "." in cleanedHeight:
                BMI2 = float(cleanedMass) / (float(cleanedHeight) ** 2) * 10000
                out[person["name"]] = round(BMI2, 2)
